...
spec = describe "parseScript" $ do
         ...
         it "should parse simple cond >" $ do
           parseScript (decodeUtf8 $(embedFile "test/fixtures/simplecond2.script")) `shouldBe` Right (Cond (Sup LaserScan 1)(Token TurnLeft) (Token MoveForward))
         it "should parse simple cond <" $ do
         ...
         it "should parse nested if cond" $ do
           parseScript (decodeUtf8 $(embedFile "test/fixtures/nestedcond.script")) `shouldBe` Right (Cond (Sup LaserDistance 10) (Cond (Sup LaserDistance 20) (Token TurnLeft) (Token TurnRight)) (Token MoveForward))
         it "should parse nested else cond" $ do
           parseScript (decodeUtf8 $(embedFile "test/fixtures/nestedcond2.script")) `shouldBe` Right (Cond (Sup LaserDistance 10)  (Token TurnLeft) (Cond (Sup LaserDistance 20) (Token TurnLeft) (Token TurnRight)))
         it "should parse deeply nested else cond" $ do
           parseScript (decodeUtf8 $(embedFile "test/fixtures/deepNestedCond.script")) `shouldBe` Right (Cond (Sup LaserDistance 10)  (Token TurnLeft) (Cond (Sup LaserDistance 20) (Cond (Inf ObjectiveDistance 3) (Token MoveForward) (Token TurnLeft)) (Token TurnRight)))
