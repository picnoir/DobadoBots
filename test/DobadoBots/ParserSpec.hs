...
spec = describe "parseScript" $ do
         ...
         it "should parse simple cond >" $
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/simplecond2.script")) `shouldBe` Right (Cond (CmpLogicInt $ Inf LaserScan 1)(Token TurnLeft) (Token MoveForward))
         it "should parse simple cond <" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/simplecond3.script")) `shouldBe` Right (Cond (CmpLogicInt $ Sup LaserDistance 1)(Token TurnLeft) (Token MoveForward))
         ...
         it "should parse weirdly indented files" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/simpleBadIndented.script")) `shouldBe` Right (Cond (CmpLogicInt $ Sup LaserDistance 1)(Token TurnLeft) (Token MoveForward))
         it "should parse tab indented files" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/simpleTabIndented.script")) `shouldBe` Right (Cond (CmpLogicInt $ Sup LaserDistance 1)(Token TurnLeft) (Token MoveForward))
         it "should parse nested if cond" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/nestedcond.script")) `shouldBe` Right (Cond (CmpLogicInt $ Inf LaserDistance 10) (Cond (CmpLogicInt $ Inf LaserDistance 20) (Token TurnLeft) (Token TurnRight)) (Token MoveForward))
         it "should parse nested else cond" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/nestedcond2.script")) `shouldBe` Right (Cond (CmpLogicInt $ Inf LaserDistance 10)  (Token TurnLeft) (Cond (CmpLogicInt $ Inf LaserDistance 20) (Token TurnLeft) (Token TurnRight)))
         it "should parse deeply nested else cond" $ 
           parseScript (decodeUtf8 $(embedFile "test/fixtures/script/deepNestedCond.script")) `shouldBe` Right (Cond (CmpLogicInt $ Inf LaserDistance 10)  (Token TurnLeft) (Cond (CmpLogicInt $ Inf LaserDistance 20) (Cond (CmpLogicInt $ Sup ObjectiveDistance 3) (Token MoveForward) (Token TurnLeft)) (Token TurnRight)))
